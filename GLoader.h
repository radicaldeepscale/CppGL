...
public:
	...
	virtual int dump(const string& fn);

	// pick up from the buffer
...
template <class _T>
int CGLoader<_T>::dump(const string& fn)
{
	return 0;
}
...
{
	cout << "Loading finished : totally " << m_numel << " vertices." << endl;
...
public:
	...
	int load(const string& fn);
	int dump(const string& fn);
	int serializeToObj(const string& fnobj);

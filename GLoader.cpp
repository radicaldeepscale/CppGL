...
int CTgdataLoader::load(const string& fn)
{
	...
	m_numel = nLine;
	report();
	cout << nLine << " polylines totally." << endl;
	unmap();
	return 0;
...
int CTgdataLoader::dump(const string& fn)
{
	...
	for (unsigned long idx = 0; idx < szTotal; ++idx) {
		...
		ofs << "curline No." << idx << " : [" << szLine << 
			" elements, " << ( szLine/nStride )	<< " points ]" << endl;
		...
		for (unsigned long j = 0; j < szLine; j += nStride) {
			...
			ofs << curLine[j+offset+0] <<  " " << 
				curLine[j+offset+1] << " " << curLine[j+offset+2] << endl;
		}
	}
...
int CSmLoader::dump(const string& fn)
{
	ofstream ofs(fn.c_str());
	if ( ! ofs.is_open() ) {
		cerr << "failed to create file : " << fn << " for serializing." << endl;
 ...
 * of WaveFront obj
 */
int CSmLoader::serializeToObj(const string& fnobj) {
